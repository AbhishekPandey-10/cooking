...
//    SafetyEngine        ~  1.2 KB  (alerts, fall FSM, RollingVar<50>)
//    Correlator           ~  0.5 KB  (45-entry window + counters)
//    FeatureExtractor     ~  2.0 KB  (peak detector, RollingVar<200>)
//    AnomalyEngine        ~  0.3 KB  (baseline stats, score tracking)
//    DegradedSos          ~  1.5 KB  (AT buffers, SMS body, payload)
//    BleRelay             ~  0.4 KB  (RX queue, dedup ring, ADV frame)
//    SensorFrame          ~  0.1 KB
//    Stack (main task)    ~  8.0 KB  (configurable via platformio.ini)
//    ──────────────────────────────
//    Total module state   ~ 14.0 KB  (well within 512 KB)
// ============================================================================
...
void setup()
{
    ...
    // ---- Modem UART --------------------------------------------------------
    ModemSerial.begin(SosConfig::MODEM_BAUD,
                      SERIAL_8N1,
                      MainConfig::MODEM_RX_PIN,
...
static void feed_sensors_to_engines(const SensorFrame &f, uint32_t now)
{
    // ---- PPG → SafetyEngine + FeatureExtractor -----------------------------
    //  feed_ppg() handles buffer-fill masking, contact-loss detection,
    //  and SpO2 threshold evaluation internally.
    //
    //  Note: The MAX30102 library computes SpO2 internally.  We pass the raw
    //  IR amplitude for signal quality gating and the computed SpO2 for
    //  threshold checks.  In this scaffold, we pass spo2 = -1 (raw IR only)
    //  and let the ppg_qualify state machine handle calibration.
    //  In production, replace with actual SpO2 from the MAX30102 algorithm.
    g_safety.feed_ppg(f.ppg.ir,
                      /* spo2 */ -1,          // TODO: wire MAX30102 SpO2 algo
                      /* spo2_valid */ false,
                      f.ppg.timestamp_ms);
...
static void run_eval_tick(uint32_t now)
{
    ...
    CorrelatorResult cr = g_correlator.tick(
        spo2,
        spo2_valid,
        g_safety.is_fall_detected(),
        g_safety.last_heat_index(),
        f.env.mq135_raw,
        mq135_valid,
        f.temp.skin_temp_c,
        now
    ...
    if ((now - s_last_status_log) >= 10000) {
        ...
        Serial.printf("[EVAL] PPG=%s SpO2=%ld Fall=%s HI=%.1f°C "
            ...
            g_safety.last_heat_index(),
            g_anomaly.anomaly_score(),
            g_anomaly.state() == AnomalyState::DORMANT  ? "DORMANT" :
            g_anomaly.state() == AnomalyState::ACTIVE   ? "ACTIVE"  : "SUSPENDED",
...
static void orchestrate_comms(uint32_t now)
{
    ...
    if (s_sos_active && g_sos.ble_fallback_needed() && !g_ble.is_broadcasting()) {
        Serial.println(F("[MAIN] Cellular exhausted — activating BLE burst broadcast"));
...
static void on_ble_relay_received(const EmergencyPayload &payload)
{
    ...
    char relay_text[32];
    snprintf(relay_text, sizeof(relay_text), "RELAY-0x%04X",
             payload.device_id);
    ...
    if (g_safety.alert_count() > 0) {
        for (size_t i = 0; i < g_safety.alert_count(); i++) {
            ...
            Serial.printf("[ALERT] %s: %s\n",
                a.severity == AlertSeverity::CRITICAL ? "CRITICAL" : "WARNING",
                a.message);
        }
    }
}
...
static void build_timestamp(char *buf, size_t cap, uint32_t uptime_ms)
{
    ...
    snprintf(buf, cap, "T+%02lu:%02lu:%02lu",
             (unsigned long)(hours % 100),
